...
void Shader::SetUniform1i(const std::string& name, int v) {
    GL_CHECK(glUniform1i(GetUniformLocation(name), v));
}
...
void Shader::SetUniform4fv(const std::string& name, unsigned int count, const float* data) {
    GL_CHECK(glUniform4fv(GetUniformLocation(name), count, data));
}

void Shader::SetUniformMatrix4fv(const std::string& name, unsigned int count, glm::mat4 matrix) {
    GL_CHECK(glUniformMatrix4fv(GetUniformLocation(name), count, GL_FALSE, glm::value_ptr(matrix)));
}
...
int Shader::GetUniformLocation(const std::string& name) {
    ...
    int location;
    GL_CHECK(location = glGetUniformLocation(rendererId_, name.c_str()));
    if (location == -1) {
