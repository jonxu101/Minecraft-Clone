    ...
    // GameTextures_()
{
    ...
    IdToCacheIndex_ = std::vector<int>(AvailableTextures_.size(), -1);
...
int TextureManager::BindBlockTexture(Utils::BlockType type, uint16_t face) {
    ...
    int cacheInd = IdToCacheIndex_[availableTextureId];
    if (cacheInd == -1) {
        std::cout << "TextureManager: Cache miss" << std::endl;
        if (CachedTextures_[cachePtr_]) { // evict in FIFO circular fashion
            IdToCacheIndex_[CachedTextures_[cachePtr_]->GetID()] = -1;
        }
        // std::cout << "erased" << std::endl;
        IdToCacheIndex_[availableTextureId] = cachePtr_;
