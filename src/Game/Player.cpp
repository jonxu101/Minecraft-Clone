...
void Player::HandleKeys() {
    ...
    for (size_t key = 0; key < GLFW_KEY_MENU; key++) {
        multiplier = Input::pressed_keys[key] ? 1.0f : 0.0f;
        switch(key) {
            case GLFW_KEY_W:
...
glm::vec3 Player::GetViewPos() {
    return glm::vec3{pos_xz[0], pos_y, pos_xz[1]};
}

glm::vec3 Player::GetViewDiff() {
    return glm::vec3{   glm::cos(view_phi) * glm::cos(view_theta), 
                        glm::sin(view_phi), 
                        glm::cos(view_phi) * glm::sin(view_theta)};
}
...
void Player::Update(std::chrono::high_resolution_clock::time_point current) {
    ...
    pos_xz += timeDelta / 1000000.0f * vel_xz;
